import {u_rectSearchX, u_rectSearchY} from "../../../../utils/geometriaAnalitica";

const u_generalDrawLine = (context, p1, p2, color, segment, lineWidth = 1) => {
...
export const u_generalConverPtoReal = (plano, p) => {
    // p: {x, y}
    let x = plano.h + (p.x) * plano.width_cuadricula;
    let y = plano.k + (- p.y) * plano.width_cuadricula;
    return {x, y};
}
export const u_generalDrawEcuacionGeneral = (context, plano, elm, width = 2) => {
    ...
    if (rec.a !== 0 && rec.b !== 0) {
        ...
        let pto2 = {x: x, y: y};
        ptoReal1 = u_generalConverPtoReal(plano, pto1);
        ptoReal2 = u_generalConverPtoReal(plano, pto2);
        u_generalDrawLine(context, ptoReal1, ptoReal2, elm.color, false, width);
    } else {
        if (rec.a === 0 && rec.b !== 0) {
